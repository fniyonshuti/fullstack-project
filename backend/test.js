...
function getTemplate$4() {
    return html`<!--_html_template_start_--><style import="cr-shared-style">:host{--action-height:28px;border:solid 1px var(--color-searchbox-results-action-chip);border-radius:8px;display:flex;height:var(--action-height);min-width:0;outline:0;padding-inline-end:8px;padding-inline-start:8px}:host(:hover){background-color:var(--color-searchbox-results-button-hover)}:host(:focus){margin:2px;border:solid 1px var(--color-searchbox-results-action-chip);box-shadow:none}.contents{align-items:center;display:flex;min-width:0}#action-icon{flex-shrink:0;-webkit-mask-position:center;-webkit-mask-repeat:no-repeat;-webkit-mask-size:15px;background-color:var(--color-searchbox-results-action-chip-icon);background-position:center center;background-repeat:no-repeat;height:16px;width:16px}#text{overflow:hidden;padding-inline-start:8px;text-overflow:ellipsis;white-space:nowrap}</style>
...
class SearchboxMatchElement extends PolymerElement {
    ...
    computeSeparatorText_() {
        return this.match && decodeString16(this.match.description) ? loadTimeData.getString("searchboxSeparator") : ""
    }
...
class PageMetricsHostRemote {
    ...
    onPageRemoteCreated(page) {
        this.proxy.sendMessage(1598257094, PageMetricsHost_OnPageRemoteCreated_ParamsSpec.$, null, [page])
    }
    onGetMark(name) {
        return this.proxy.sendMessage(922208845, PageMetricsHost_OnGetMark_ParamsSpec.$, PageMetricsHost_OnGetMark_ResponseParamsSpec.$, [name])
    }
    onClearMark(name) {
        this.proxy.sendMessage(1017361901, PageMetricsHost_OnClearMark_ParamsSpec.$, null, [name])
    }
    onUmaReportTime(name, time) {
        this.proxy.sendMessage(2041211390, PageMetricsHost_OnUmaReportTime_ParamsSpec.$, null, [name, time])
    }
}
...
class PageMetricsRemote {
    ...
    onGetMark(name) {
        return this.proxy.sendMessage(1927659820, PageMetrics_OnGetMark_ParamsSpec.$, PageMetrics_OnGetMark_ResponseParamsSpec.$, [name])
    }
    onClearMark(name) {
        this.proxy.sendMessage(844375285, PageMetrics_OnClearMark_ParamsSpec.$, null, [name])
    }
}
class PageMetricsCallbackRouter {
    ...
    constructor() {
        ...
        this.helper_internal_.registerHandler(1927659820, PageMetrics_OnGetMark_ParamsSpec.$, PageMetrics_OnGetMark_ResponseParamsSpec.$, this.onGetMark.createReceiverHandler(true));
        this.onClearMark = new mojo.internal.interfaceSupport.InterfaceCallbackReceiver(this.router_);
        this.helper_internal_.registerHandler(844375285, PageMetrics_OnClearMark_ParamsSpec.$, null, this.onClearMark.createReceiverHandler(false));
...
<div id="inputWrapper" on-focusout="onInputWrapperFocusout_" on-keydown="onInputWrapperKeydown_">
  ...
  <cr-searchbox-icon id="icon" match="[[selectedMatch_]]" default-icon="[[searchboxIcon_]]" in-searchbox>
  </cr-searchbox-icon>
  ...
  <template is="dom-if" if="[[searchboxChromeRefreshTheming]]">
    <template is="dom-if" if="[[searchboxVoiceSearchEnabled_]]">
      <div class="searchbox-icon-button-container voice">
        <button id="voiceSearchButton" class="searchbox-icon-button" on-click="onVoiceSearchClick_" title="[[i18n('voiceSearchButtonLabel')]]">
        </button>
      </div>
    </template>
    <template is="dom-if" if="[[searchboxLensSearchEnabled_]]">
      <div class="searchbox-icon-button-container lens">
          <button id="lensSearchButton" class="searchbox-icon-button" on-click="onLensSearchClick_" title="[[i18n('lensSearchButtonLabel')]]">
          </button>
      </div>
    </template>
  </template>
...
function getHtml$2() {
  ...
  <div slot="body">
    <div id="buttons">
      ...
      <cr-button id="emailButton" title="E-mail" @click="${this.onEmailClick_}">
      </cr-button>
    </div>
    <cr-input readonly="readonly" label="Doodle Link" id="url" .value="${this.url.url}">
      <cr-icon-button id="copyButton" slot="suffix" title="Copy Link" @click="${this.onCopyClick_}">
      </cr-icon-button>
    </cr-input>
  </div>
...
${this.showDoodle_ ? html$1`
  <div id="doodle" title="${this.doodle_.description}">
    <div id="imageDoodle" ?hidden="${!this.imageDoodle_}" tabindex="${this.imageDoodleTabIndex_}" @click="${this.onImageClick_}" @keydown="${this.onImageKeydown_}">
      ...
      <cr-button id="shareButton" title="Share Doodle" @click="${this.onShareButtonClick_}">
...
function getHtml() {
  ...
  <div id="searchboxContainer">
    <cr-searchbox id="searchbox" ?is-dark="${this.isThemeDark_()}" ?color-source-is-baseline="${this.colorSourceIsBaseline}" @open-lens-search="${this.onOpenLensSearch_}" @open-voice-search="${this.onOpenVoiceSearch_}" ?shown="${this.realboxShown_}" ?had-secondary-side="${this.realboxHadSecondarySide}" @had-secondary-side-changed="${this.onRealboxHadSecondarySideChanged_}" ?can-show-secondary-side="${this.realboxCanShowSecondarySide}">
    </cr-searchbox>
  ...
  ${this.lazyRender_ ? html$1`
    ...
    <div id="customizeButtons">
      ${this.showWallpaperSearchButton_ ? html$1`
        <cr-button id="wallpaperSearchButton" class="customize-button" @click="${this.onWallpaperSearchClick_}" title="Customize this page with AI" aria-pressed="${this.showWallpaperSearch_}">
      ...
      ` : ""}
      <cr-button id="customizeButton" class="customize-button" @click="${this.onCustomizeClick_}" title="Customize this page" aria-pressed="${this.showCustomize_}">
...
class AppElement extends AppElementBase {
    ...
    constructor() {
        ...
        this.searchboxWidthBehavior_ = loadTimeData.getString("searchboxWidthBehavior");
